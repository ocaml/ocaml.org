...
let papers _req =
  ...
  Page_layout_template.render ~title:"Papers" (Papers_template.render papers)
  |> Dream.html
...
let books _req =
  ...
  Page_layout_template.render ~title:"Books" (Books_template.render books)
  |> Dream.html

let events _req =
  let events = Ood.Event.all in
  Page_layout_template.render ~title:"Events" (Events_template.render events)
  |> Dream.html

let videos _req =
  let videos = Ood.Video.all in
  Page_layout_template.render ~title:"Videos" (Videos_template.render videos)
  |> Dream.html
...
let workshops req =
  ...
  Dream.redirect req ("/preview/workshops/" ^ slug)

let tools _req =
  let tools = Ood.Tool.all in
  Page_layout_template.render ~title:"Tools" (Platform_template.render tools)
  |> Dream.html

let news _req =
  let news = Ood.News.all in
  Page_layout_template.render ~title:"News" (News_template.render news)
  |> Dream.html
