...
let compare_list ~f =
  let rec iter = function
    ...
    | _, [] ->
      1
    | h1 :: q1, h2 :: q2 ->
      (match f h1 h2 with 0 -> iter (q1, q2) | n -> n)
  in
...
let compare_guid g1 g2 =
  ...
  | Guid_permalink url1, Guid_permalink url2 ->
    Uri.compare url1 url2
  | Guid_permalink _, Guid_name _ ->
    1
  | Guid_name _, Guid_permalink _ ->
    -1
  | Guid_name s1, Guid_name s2 ->
    compare s1 s2

let compare_source s1 s2 =
  match Uri.compare s1.src_url s2.src_url with
  | 0 ->
    compare s1.src_name s2.src_name
  | n ->
    n
...
let compare_category c1 c2 =
  ...
  | 0 ->
    compare c1.cat_name c2.cat_name
  | n ->
    n
...
let compare_item ?comp_data =
  let comp_funs =
    ...
    | None ->
      item_comp_funs
    | Some f ->
      (fun i1 i2 -> compare_opt ~f i1.item_data i2.item_data) :: item_comp_funs
  in
