...
let to_yaml video =
  match Data_packer.Vid.to_yaml video with
...
let of_yaml yml =
  yml |> add_key_default "description" (`String "") |> Data_packer.Vid.of_yaml
...
module VideoSet = Set.Make (struct
  ...
  let compare a b = compare a.Data_packer.Vid.url b.Data_packer.Vid.url
end)
