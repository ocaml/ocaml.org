...
module Conv : sig
  ...
  let rec to_jv rpc =
    ...
    match rpc with
    | Rpc.Int i ->
      mk "int" (Jv.of_int (Int64.to_int i))
    | Int32 i ->
      mk "int32" (Jv.of_int (Int32.to_int i))
    | Bool b ->
      mk "bool" (Jv.of_bool b)
    | Float f ->
      mk "float" (Jv.of_float f)
    | String s ->
      mk "string" (Jv.of_string s)
    | DateTime s ->
      mk "datetime" (Jv.of_string s)
    | Enum xs ->
      mk "enum" (Jv.of_jv_list (List.map to_jv xs))
    | Dict xs ->
  ...
  let rec of_jv jv =
    ...
    match ty with
    | "int" ->
      Rpc.Int (Int64.of_int (Jv.to_int v))
    | "int32" ->
      Rpc.Int32 (Int32.of_int (Jv.to_int v))
    | "bool" ->
      Rpc.Bool (Jv.to_bool v)
    | "float" ->
      Rpc.Float (Jv.to_float v)
    | "string" ->
      Rpc.String (Jv.to_string v)
    | "datetime" ->
      Rpc.DateTime (Jv.to_string v)
    | "enum" ->
      Rpc.Enum (Jv.to_list of_jv v)
    | "dict" ->
      let keys = Jv.to_list Jv.to_string (Jv.get v "__keys__") in
      let kvs = List.map (fun k -> k, of_jv (Jv.get v k)) keys in
      Rpc.Dict kvs
    | "base64" ->
      Base64 (Jv.to_string v)
    | "null" ->
      Null
    | _ ->
