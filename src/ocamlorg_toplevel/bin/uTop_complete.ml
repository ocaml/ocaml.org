...
let lookup_assoc word words = List.filter (fun (word', _) -> Astring.String.is_prefix ~affix:word word') words
let lookup word words = List.filter (fun word' -> Astring.String.is_prefix word' ~affix:word) words
...
let list_directives phrase_terminator =
  String_map.bindings
    (List.fold_right
       (fun dir map ->
          let kind = Toploop.get_directive dir in
          let suffix =
            match kind with
...
let basename name =
  ...
  if name' = "." && not (Astring.String.is_suffix name ~affix:".") then
    ""
  else
    name'
...
let complete ~phrase_terminator ~input =
  ...
  match tokens with
    ...
    | [(Symbol "#", _); ((Lident _ | Uident _), _); (String (_, true), _); (Symbol sym, { idx1 = start })] ->
        if Astring.String.is_prefix phrase_terminator ~affix:sym then
    ...
    | [(Symbol "#", _); (Lident ("cd" | "directory"), _); (String (tlen, false), loc)] ->
        ...
        (loc.idx2 - String.length name, List.map (function dir -> (dir, "")) result)

    (* Generic completion on directives. *)
    | [(Symbol "#", _); ((Lident dir | Uident dir), _); (Blanks, { idx2 = stop })] ->
        (stop,
         match Toploop.get_directive dir with
           | Some (Toploop.Directive_none _) -> [(phrase_terminator, "")]
           | Some (Toploop.Directive_string _) -> [(" \"", "")]
