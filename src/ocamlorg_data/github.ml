...
let input_line cin = try Some (Stdlib.input_line cin, cin) with End_of_file -> close_in cin; None

let read filepath =
  filepath
  |> open_in
  |> Seq.unfold input_line
  |> Seq.fold_left (^) ""
...
let request token repo =
  ...
  let rec loop buf graphql =
    ...
    let has_previous_page, start_cursor = page_info (json |> !! "pullRequests") in
...
let request token org =
  ...
  Yojson.Safe.Util.{
    name = json |> !! "name" |> to_string;
    description = json |> !! "description" |> to_string_option;
    members = json |> !! "membersWithRole" |> !! "edges" |> convert_each to_member;
