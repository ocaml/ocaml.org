...
let post token graphql =
  ...
  | `OK -> Ok Yojson.Safe.(body |> from_string |> Util.member "data")
  | _ -> Error (`Msg body)

  let repository ?before repo =
    let before = match before with
    | Some before -> ", before: \"" ^ before ^ "\""
    | None -> "" in
    "{
...
module Yojson = Yojson.Safe.Util
...
let request_repo token repo =
  let (let*) = Lwt_result.bind in
  let (let+) = Fun.flip Lwt_result.map in
  let (!!) = Yojson.member in
  let rec loop buf graphql =
    ...
    Lwt_result.map (List.cons json) @@ if has_previous_page then
      loop buf (repository ~before:start_cursor repo)
    else
      Lwt.return_ok buf in
  let+ list = loop [] (repository repo) in
