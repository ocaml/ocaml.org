      ...
      ( "test parsing and path generation",
        [
          test_case "simple json" (fun () ->
              ...
              let t = Package_info.of_yojson json in
              Alcotest.(check int)
                "single library"
              ...
              Alcotest.(check string)
                ...
                (Package_info.Module.name module');
              Alcotest.(check int)
                "4 submodules" 4
                (String.Map.cardinal (Package_info.Module.submodules module'));
              Alcotest.(check (list string))
                ...
                (Package_info.Module.submodules module'
                |> String.Map.bindings
                |> List.map (fun (_, v) -> Package_info.Module.path v)));
        ] );
    ]
