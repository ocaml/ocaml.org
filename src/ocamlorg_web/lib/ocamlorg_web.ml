...
let run ~interface ~port f =
  ...
  let scheme = if Config.https_enabled then "https" else "http" in
  (match interface with
  | "localhost" | "127.0.0.1" -> log "Running at %s://localhost:%i" scheme port
  | _ -> log "Running on %s:%i (%s://localhost:%i)" interface port scheme port);
...
let http () =
  ...
  @@ Dream.logger @@ Le.router @@ redirect_handler
...
let server https port =
  ...
  if not https then
    ...
    (* Use Dream's development certificate if we are running the server
       locally. *)
    Dream.serve ~https:true ~interface:"0.0.0.0" ~debug:Config.debug
    ...
    let* certificate_file, key_file =
      Le.load_certificates ~certificate_file_path:Config.certificate_file_path
        ~private_key_file_path:Config.private_key_file_path
        ~hostname:Config.hostname ~staging:Config.letsencrypt_staging ()
    in
...
let run () =
  ...
  else server Config.https_enabled port
