...
let packages_search t req =
  let packages =
    match Dream.query req "q" with
    | Some search -> Ocamlorg_package.search ~is_author_match ~sort_by_popularity:true t search
  ...
  in
  let search = Dream.from_percent_encoded (match Dream.query req "q" with | Some search -> search | None -> "") in
