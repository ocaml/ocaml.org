...
let provision_certificate ?email ?seed ?cert_seed ?hostname ~staging () =
  let endpoint =
    if staging then Letsencrypt.letsencrypt_staging_url
    else Letsencrypt.letsencrypt_production_url
  in
...
let save_certificate_files ~certificate_file_path ~private_key_file_path
  ...
  in
  Logs.info (fun m ->
      m "saving certificate file with content %s to %s" certificate_s
  ...
  in
  Logs.info (fun m ->
      m "saving certificate file with content %s to %s" private_key_s
        certificate_file_path);
...
let provision_and_save ?email ?seed ?cert_seed ~hostname ~staging
  ...
  let+ certificates, private_key =
    ...
    | Error (`Msg err) ->
        failwith
          (Printf.sprintf "Could not get the certificate from letsencrypt %s"
             err)
  in
  save_certificate_files ~certificate_file_path ~private_key_file_path
    certificates private_key

let load_certificates ~certificate_file_path ~private_key_file_path ~hostname
    ~staging ?email ?seed ?cert_seed () =
  let open Lwt.Syntax in
  if
  ...
  then
    ...
    | _ ->
        let+ () =
          provision_and_save ?email ?seed ?cert_seed ~hostname ~staging
            ~certificate_file_path ~private_key_file_path ()
        in
        (certificate_file_path, private_key_file_path)
  else
