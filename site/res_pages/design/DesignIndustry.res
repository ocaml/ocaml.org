...
let pageOverview = s(`Ocaml is a popular choice for companies who make use of its features in key
...
let facebookBody = s(`Facebook has built a number of major development tools using OCaml. Hack 
...
let dockerBody = s(`Docker provides an integrated technology suite that enables development and IT operations 
...
let taridesBody = s(`We are building and maintaining open-source infrastructure tools in OCaml: (1) MirageOS, 
...
let solvuuBody = s(`Solvuu's software allows users to store big and small data sets, share the data with 
