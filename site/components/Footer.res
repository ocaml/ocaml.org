...
module MutedLink = {
  ...
  let make = (~href, ~text) =>
    <Link href>
      <a className="text-base text-gray-500 hover:text-gray-900"> {s(text)} </a>
    </Link>
}
      ...
      <ul className="mt-4 space-y-4">
        {
          keyPages
          |> Js.Array.map(p => 
            <li><MutedLink href=p.NavEntry.url text=p.NavEntry.label /></li>)
          |> React.array
        }
      </ul>
    </div>
...
module BasicLink = {
  ...
  let make = (~href, ~text) =>
    <Link href>
      <a className="text-orangedark underline"> {s(text)} </a>
    </Link>
}
