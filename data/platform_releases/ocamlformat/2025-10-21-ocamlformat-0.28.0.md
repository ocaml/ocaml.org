...
We're happy to announce the release of OCamlFormat `0.28.0`!

## Language Support

* **OCaml 5.4:** The tool now supports OCaml 5.4 syntax.
* **Module Packing:** To distinguish between different syntaxes, module packing of the form `((module M) : (module S))` is no longer rewritten to `(module M : S)`.

## Configuration Changes

* **`module-indent`:** A new option to control the indentation of items within modules and signatures.
* **`exp-grouping`:** The default value is now `preserve` for the `default` and `ocamlformat` profiles. This allows users to use `begin ... end` blocks without them being removed by the formatter.
* **`break-struct`:** The `natural` setting now applies to signatures (`sig ... end`) in addition to structures.

## Formatting Adjustments

* **Indentation:** Indentation after `|> map (fun` no longer scales with the length of the infix operator.
* **Vertical Space:** Several constructs, including `begin fun`, `begin match`, and `begin if`, can now be printed on the same line to reduce vertical nesting.
* **Infix Consistency:** * Formatting for infix extensions and regular expressions is now consistent.
* Spacing for infix attributes is standardized; for example, `fun[@a]` is used instead of `fun [@a]`.
* **Top-level `;;`:** Fixed an issue where `;;` was incorrectly handled near documentation comments, resulting in more consistent insertion of the separator.

## Command Line Interface (CLI)

* **Cmdliner Compatibility:** This version supports `cmdliner >= 2.0.0`. When using this version, the CLI no longer accepts shortened or ambiguous option names. You must provide the full flag (e.g., `--max-iters` instead of `--max-iter`). This change does not affect `.ocamlformat` configuration files.

## Bug Fixes

* **Profiles:** Resolved an issue where `wrap-comments=true` did not work correctly within the `janestreet` profile.
* **Parsing and Logic:** Fixed crashes related to `mld` files containing non-OCaml code (like LaTeX or C), asterisk-prefixed comments, and specific GADT syntax.
* **AST Corrections:** Improved AST generation for specific `let` bindings and module constraints in functor applications.
