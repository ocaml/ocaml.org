...
This release adds support for OCaml 5.5.0 ([#1406](https://github.com/ocaml/odoc/pull/1406)) and for the OxCaml compiler variant ([#1399](https://github.com/ocaml/odoc/pull/1399)). The HTML/KaTeX backend now supports persistent LaTeX macros ([#1391](https://github.com/ocaml/odoc/pull/1391)), so a macro defined with `\gdef` in one math block can be reused in subsequent ones, and the `markdown-generate` command now accepts multiple `.odocl` files in a single invocation, removing the need for shell scripting around it ([#1388](https://github.com/ocaml/odoc/pull/1388)).
