...
The default subcommand is *list* if there are no further arguments, and
...
Without argument, defaults to **list**.
  ...
  - **add** *PACKAGE* *TARGET*  
    ...
    a package by that name is already known. If *TARGET* is **-**, the
    package is pinned as a virtual package, without any source. opam
    will infer the kind of pinning from the format (and contents, if
    local) of *TARGET*, Use **--kind** or an explicit URL to disable
    that behaviour. Pins to version control systems may target a
    specific branch or commit using **\#branch** e.g.
...
## OPTIONS
  ...
  - **-k** *KIND*, **--kind**=*KIND*  
    ...
    *OPAMPINKINDAUTO* can be set to "0" to disable automatic detection
    of version control.Use *none* to pin without a target (for virtual
    packages).
  - **-n**, **--no-action**  
