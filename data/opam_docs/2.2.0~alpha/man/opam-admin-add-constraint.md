---
slug: opam-admin-add-constraint.html
title: Manpage opam-admin-add-constraint
description: |
  man page on opam-admin-add-constraint.
---
...
opam admin-add-constraint - Adds version constraints on all dependencies
towards a given package <span id="lbAC"> </span>

## SYNOPSIS

**opam admin add-constraint** \[**--force**\]
\[**--packages**=*PACKAGES*\] \[*OPTION*\]… *PACKAGE*
<span id="lbAD"> </span>

## DESCRIPTION

This command searches to all dependencies towards a given package, and
...
constraint is merged with the existing one, and simplified if possible
(e.g. **\>=3 & \>5** becomes **\>5**). <span id="lbAE"> </span>

## ARGUMENTS

  - *PACKAGE* (required)  
    A package name with a version constraint, e.g. **name\>=version**.
...
## OPTIONS
  ...
  - **--force**  
    ...
    unsatisfiable (e.g. when adding **\>3** to the constraint **\<2**).
    The default in this case is to print a warning and keep the existing
    constraint unchanged.
  - **-p** *PACKAGES*, **--packages**=*PACKAGES*  
    Only add constraints for the given packages
...
opam [admin](../man1/admin.1.html)(1) <span id="lbAO"> </span>
