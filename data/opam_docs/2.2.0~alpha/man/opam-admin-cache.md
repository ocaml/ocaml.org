---
slug: opam-admin-cache.html
title: Manpage opam-admin-cache
description: |
  man page on opam-admin-cache.
---
# OPAM ADMIN-CACHE
...
**opam admin cache** \[*OPTION*\]… \[*DIR*\] <span id="lbAD"> </span>

## DESCRIPTION

Downloads the archives for all packages to fill a local cache, that can
be used when serving the repository. <span id="lbAE"> </span>

## ARGUMENTS

  - *DIR* (absent=**\~/opam/\_build/default/doc/man/cache**)  
...
## OPTIONS
  ...
  - **-c**, **--check-all**  
    Run a full integrity check on the existing cache. If this is not
    set, only missing cache files are handled.
  - **-j** *JOBS*, **--jobs**=*JOBS* (absent=**8**)  
    Number of parallel downloads
  - **--link**=*DIR*  
    Create reverse symbolic links to the archives within *DIR*, in the
    form **DIR/PKG.VERSION/FILENAME**.
  - **-n**, **--no-repo-update**  
