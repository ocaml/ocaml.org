---
slug: opam-admin.html
title: Manpage opam-admin
description: |
  man page on opam-admin.
---
# OPAM ADMIN
...
**opam admin** \[*COMMAND*\] … <span id="lbAD"> </span>
...
the **upgrade-format** subcommand. <span id="lbAE"> </span>

## COMMANDS

  - **add-constraint** \[**--force**\] \[**--packages**=*PACKAGES*\]
    \[*OPTION*\]… *PACKAGE*  
    Adds version constraints on all dependencies towards a given package
  - **add-hashes** \[**--packages**=*PACKAGES*\] \[**--replace**\]
    \[*OPTION*\]… *HASH\_ALGO*…  
    Add archive hashes to an opam repository.
  - **cache** \[*OPTION*\]… \[*DIR*\]  
    Fills a local cache of package archives
  - **check** \[*OPTION*\]…  
    Runs some consistency checks on a repository
  - **filter** \[**--dry-run**\]
    \[**--environment**=*VAR=VALUE\[,VAR=VALUE\]*\] \[**--remove**\]
    \[*OPTION*\]… \[*PATTERNS*\]…  
    Filters a repository to only keep selected packages
  - **help** \[**--man-format**=*FMT*\] \[*OPTION*\]… \[*TOPIC*\]  
    Display help about opam admin and opam admin subcommands.
  - **index** \[**--full-urls-txt**\] \[**--minimal-urls-txt**\]
    \[**--no-urls-txt**\] \[*OPTION*\]…  
    Generate an inclusive index file for serving over HTTP.
  - **lint** \[*OPTION*\]… \[*INT*\]…  
    Runs 'opam lint' and reports on a whole repository
  - **list** \[**--environment**=*VAR=VALUE\[,VAR=VALUE\]*\]
    \[*OPTION*\]… \[*PATTERNS*\]…  
    Lists packages from a repository
  - **upgrade** \[**--clear-cache**\] \[**--mirror**=*URL*\]
    \[*OPTION*\]…  
...
## COMMAND ALIASES
  ...
  - **make** \[**--full-urls-txt**\] \[**--minimal-urls-txt**\]
    \[**--no-urls-txt**\] \[*OPTION*\]…  
    An alias for **index**.
