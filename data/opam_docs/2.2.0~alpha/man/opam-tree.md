---
slug: opam-tree.html
title: Manpage opam-tree
description: |
  man page on opam-tree.
---
# OPAM-TREE
...
opam-tree - Draw the dependency forest of installed packages.
<span id="lbAC"> </span>

## SYNOPSIS

**opam tree** \[*OPTION*\]… \[*PACKAGES*\]… <span id="lbAD"> </span>

## DESCRIPTION

This command displays the forest of currently installed packages as a
Unicode/ASCII-art.

Without argument, it draws the dependency forest of packages with no
dependants. With packages arguments, draws the forest of the specified
packages. When non-installed packages are specified in the arguments, it
will try to simulate installing them before drawing the forest.

When the **--rev-deps** option is used, it draws the reverse-dependency
forest instead. Without argument, draws the forest of packages with no
dependencies. With packages arguments, draws the forest of the specified
packages. Note that non-installed packages are ignored when this option
is used.

When a package appears twice or more in the forest, the second or later
occurrences of the said package will be marked as a duplicate, and be
annotated with the *\[\*\]* symbol. Any sub-trees rooted from such
duplicates will be truncated to avoid redundancy.

See section **TREE FILTERING OPTIONS** and **PACKAGE SELECTION OPTIONS**
for all the ways to select the packages to be displayed, and section
**OUTPUT FORMAT OPTIONS** to customise the output format.

For a flat list of packages which may not be installed, see **opam
list**. <span id="lbAE"> </span>
...
These options only take effect when *PACKAGES* are present.
...
## Index
  ...
  - [TREE FILTERING OPTIONS](#lbAG)

  - [PACKAGE SELECTION OPTIONS](#lbAH)

  - [OUTPUT FORMAT OPTIONS](#lbAI)
...
the manual pages.  
Time: 11:35:03 GMT, July 19, 2023
