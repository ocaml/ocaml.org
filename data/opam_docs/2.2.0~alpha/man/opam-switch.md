...
**opam switch set** sets the default switch globally. The shell hook,
when enabled, synchronises the current shell session with this switch,
unless the current directory is a local switch, when that local switch
is used instead. You can always use *eval $(opam env --switch=SWITCH
--set-switch)* to specify the switch explicitly, which overrides the
shell hook. <span id="lbAE"> </span>
...
Without argument, defaults to **list**.
  ...
  - *-*  
    Switches back to the previous non-local switch (similar to **git
    switch -**). To set the current switch to a switch named *-* use
    **--cli=2.1** or lower, or use **opam switch set -**
...
## OPTIONS
  ...
  - **--formula**=*FORMULA*  
    ...
    including disjunction cases, as the switch invariant. The format is
    the same as for expressing dependencies in package definition files,
    e.g. '"foo" {\>= "1.1"}'
  - **--freeze**  
