---
slug: opam-option.html
title: Manpage opam-option
description: |
  man page on opam-option.
---
# OPAM-OPTION
...
**opam option** \[**--global**\] \[*OPTION*\]…
\[*FIELD\[(=|+=|-=)\[VALUE\]\]*\] <span id="lbAD"> </span>

## DESCRIPTION

Without argument, list all configurable fields. If a field name *FIELD*
is given, display its content. Otherwise, sets or updates the given
field in the global/switch configuration file. For global configuration,
*FIELD* is reset to its default initial value, as after a fresh init
(use \`opam init show-default-opamrc\` to display it).
<span id="lbAE"> </span>

## ARGUMENTS

  - *FIELD\[(=|+=|-=)\[VALUE\]\]*  
    If only *FIELD* is given, displays its associated value. If *VALUE*
    is absent, *FIELD*'s value is reverted. Otherwise, its value is
    updated: overwrite (=), append (+=), or remove of an element (-=).
