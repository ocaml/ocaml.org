...
- **OCaml 5.5.0 reaches beta** (Apr 20): a relocatable compiler plus modular explicits, polymorphic function parameters, generalised local bindings, and garbage-collector improvements; an rc1 followed, and the final OCaml 5.5.0 was released on June 19, 2026, just after this newsletter's February–May window.
- **Security**: OCaml 5.4.1 / 4.14.3 (Feb 17) harden Marshal against malicious input (OSEC-2026-01) and opam 2.5.1 (Apr 16) blocks `.install` path escapes (OSEC-2026-03). Review your code and upgrade if you deserialise untrusted data, or upgrade if you maintain a distribution. You can also [check the published security issues](https://osv.dev/list?q=&ecosystem=opam) and upgrade if you use an affected package, and [subscribe to security announcements](https://ocaml.org/security#mailing-list-for-security-announcements).
...
The `MSet.elt` in the inferred type refers back to the module argument, so the return type depends on the module passed in. Application is more restricted than for ordinary first-class-module functions: the argument must be a literal module expression like `(module IntSet)`, not a value computed at runtime. This restriction is what keeps the typing sound. The manual documents the feature ([#14048](https://github.com/ocaml/ocaml/pull/14048)). The related polymorphic function parameters feature (`let f (g : 'a. 'a t -> 'a) = ...`) lands in the same release.
