...
To create a module, let's create a new file named `lib/world.ml` containing this:
```ocaml
let v = "Hello from a module"
```
...
**Note**: If you create a file called `lib/hello.ml`, Dune will consider this the whole `Hello` module and will make `World` unreachable. <!-- FIXME: Refer to the [Libraries With Dune](/docs/libraries-dune) tutorial for more information on this. -->
...
Then, inside the `utop` toplevel, it is possible to inspect our `Hello.World` module:
```ocaml
# #show Hello.World;;
...
**Note**: If you add a file named `hello.ml` in the `lib` folder, it replaces the module `Hello` generated by Dune by the module from your `hello.ml` file. If you want your module `World` to be visible, you need to add this in your `hello.ml` file:
```ocaml
module World = World
```
...
UTop's `#show` command displays an [API](https://en.wikipedia.org/wiki/API#Libraries_and_frameworks) (in the software library sense): the list of definitions provided by a module. In OCaml, this is called a _module interface_. An `.ml` file defines a module. In a similar way, an `.mli` file defines a module interface. The module interface file corresponding to a module file must have the same base name, e.g., `world.mli` is the module interface for module `world.ml`. Create a `lib/world.mli` file with this content:
```ocaml
val v : string
```

Observe that only the list of declarations of the module signature (which is between `sig` and `end` in the output from `#show`) has been written in the interface file `lib/world.mli`. This is explained in more detail in the tutorial dedicated to [modules](/docs/modules).
...
In your preferred editor, amend the `lib/world.ml` file to add the `mundo` definition. Replace what's there with the following:
...
1 | let () = Printf.printf "%s\n" Hello.World.mundo
                                  ^^^^^^^^^^^
Error: Unbound value Hello.World.mundo
```
...
```ocaml
let () = Dream.(run (router [ get "/" (fun (_ : request) -> html Hello.World.v) ]))
```

This gives us a web server that responds with the content of `Hello.World.v` to HTTP requests to the '/' path. Refer to the [Dream documentation](https://aantron.github.io/dream/) for more information.
...
- `html Hello.World.v` has type `response promise`. When data inside the promise becomes available, the server will send it, too. In our case, it is immediately available as it is a static constant stored in memory.

In summary, this is telling: “run a web server responding with the content of `Hello.World.v` to requests to the '/' path”
...
The files `lib/world.ml` and `lib/world.mli` need to be edited, too:
...
```ocaml
let () = print_endline Hello.World.(string_of_string_list v)
```
