...
**Note 4**: Unlike in Unix where they compiled binaries, folders `lib` and `bin` contain source code files, for libraries and programs, respectively. This is the a convention used in many OCaml projects, and the canned project created by Dune. All the built artifacts, and a copy of the sources, are stored in the `_build` folder. You shall not edit anything in the `_build` folder.
```shell
$ cd hello
$ tree
.
