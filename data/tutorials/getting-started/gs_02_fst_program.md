...
Although `bin/main.ml`'s name accurately suggests it contains the application entry point into the executable binary produced by the project, it does not contain a dedicated `main` function. Even that file name is irrelevant, it could be anything. This is because OCaml is equally behaves as an interpreted and a compiled language. The toplevel reads input, type checks it, compiles it and executes it, triggering computation and side effects. Execution of the binaries generated by the compiler behaves as if interpretation was taking place, the only difference should be speed.
