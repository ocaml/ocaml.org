...
However, it is the habit to single out a value triggering all the side-effects and mark it as the intended main entry point. That's the role of the `let () =` pattern which stands for “Definition without a name” and actulally indicates we're only interrested in the side effects of the value on the right.
