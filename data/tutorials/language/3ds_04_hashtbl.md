...
The OCaml Standard Library's `Hashtble` module implements an associative array
...
hashing function within the `Hashtable` module that is available for each key
...
The 123456 is the initial element size of the hash table. This initial number is
just your best guess as to the amount of data that you will be putting into the
hash table. The hash table can grow if you under-estimate the size so don't
worry about it too much. The type of my_hash is:
...
slots because the type of the key and value are not yet undetermined. The
