...
Dune stores the files it creates, and a copy of the sources, in a folder named `_build`. You don't need to edit anything there. In a project managed using Git, the `_build` folder should be ignored
```shell
...
Dune creates libraries from folders. Let's look at an example. Here the folder `lib` will contain its sources. This is different from the Unix standard, where `lib` stores compiled library binaries.
```shell
$ mkdir lib
```

The `lib` folder is populated with the following source files:
