...
64-bit host, or use the [`Bigarray`](/manual/latest/api/Bigarray.html) module.
...
: The `Bigstring` module provides a `String`-like interface that uses
  [`Bigarray`](/manual/latest/api/Bigarray.html) internally. The `Bigbuffer` collects these into extensible
