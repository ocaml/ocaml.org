...
using [Playground](https://ocaml.org/play), with no installation required.
...
The in-browser [Playground](https://ocaml.org/play) has a similar interface.

The examples in this tutorial can be typed in by hand, or copied into `ocaml`,
`utop`, or Playground with copy and paste. Alternatively, we may type into a file
...
to follow the tutorial moving forward, not just the Playground tool.
