...
(* Sum of two vectors u and v: the motivation of this definition is that although the element (ij) of the matrix a+b is a.(i).(j) + b.(i).(j), we also can add matrices directly row by row (which are vectors). *)
let add_vec u v = Array.map2 ( + ) u v

(* Dot product of two vectors u and v: will be useful because the element (i,j) of the multiplication of two matrices a and b can be defined as the dot product of the i-row of a by the j-column of b. *)
