...
- filename: main.ml
  ...
  - explanation: |
      Select query: the first `SELECT` query has a `get_many` type, then, it will return a list of values. Each item of the list is a record, as specified by the `record_out` tag. Output values (generated by the query) are annotated with a `@` notation. A second has output values, and an input value used in the `WHERE` clause. (Here the absence of `record_in` imply passing input values as named arguments instead of a record, the `get_opt` tag indicates it will return an option type: `None` if nothing is found, `Some r` if a row match the criteria).
    code: |
      let select_query =
        [%rapper
