...
(* 
Defining a custom `compare` function (here, a case insensitive string comparison) and using it to compare.
 Note: the comparison function applied to `a` and `b` should return 1 if `a` is greater, 0 if equal, and -1 if lower than `b`.
*)
