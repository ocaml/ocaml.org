...
- name: yojson
  ...
  - ppx_yojson_conv
---
(* We need to open `Ppx_yojson_conv_lib.Yojson_conv.Primitives` in order to serialise values of primitive types. *)
...
(* First we convert the JSON string to `Yojson.Safe.t` and then we use the generated function to create the record.
   If the parsing fails the result will be None. Otherwise it will be the deserialised record.*)
let ocaml_language = { 
  name: "ocaml"; 
  url: "https://ocaml.org/" 
}
